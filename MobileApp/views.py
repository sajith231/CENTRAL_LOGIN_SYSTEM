...
def add_mobile_control(request):
    ...
    if request.method == 'POST':
        ...
        if not project_id or not customer_name or not client_id:
            ...
            return render(request, "add_mobile_control.html", {'projects': projects})

        try:
            project = MobileProject.objects.get(pk=int(project_id))
        except (MobileProject.DoesNotExist, ValueError):
...
def edit_mobile_control(request, pk):
    ...
    if request.method == 'POST':
        ...
        if not project_id or not customer_name or not client_id:
            messages.error(request, 'Project, Customer name and Client ID are required.')
            return render(request, "edit_mobile_control.html", {'control': control, 'projects': projects})
